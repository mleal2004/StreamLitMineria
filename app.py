...
if row_variable and col_variable:
    try:
        pivot = data.pivot_table(index=row_variable, columns=col_variable, aggfunc="size", fill_value=0)
...
if len(numeric_columns) > 0:
    ...
    sns.histplot(data[column_to_plot], kde=True, color="blue", ax=ax)
...
if len(categorical_columns) > 0:
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    sns.countplot(data=data, x=column_to_plot, palette="viridis", ax=ax)
